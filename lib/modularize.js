...
Modularize.prototype.configure = function(grunt, task) {
  return this.formatter.configure(grunt, task);
};
...
Modularize.prototype.module = function(content) {
  var submodules  = this.parse(content);
  var current     = submodules.shift();

  return this.format(current, submodules, content);
};
